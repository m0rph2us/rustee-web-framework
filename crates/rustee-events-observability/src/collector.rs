...
use super::EventMetricsSnapshot;
...
pub const DEFAULT_EVENT_DELIVERY_DURATION_BUCKETS: [Duration; 12] = DEFAULT_DURATION_BUCKETS;
...
/// provider labels. Event type, ID, topic, partition, offset, key, trace context, and error text
/// are deliberately absent from labels.
#[derive(Clone, Debug)]
pub struct EventMetrics {
    pub(super) state: Arc<Mutex<EventMetricsState>>,
}
...
impl EventMetrics {
    ...
    pub fn with_duration_buckets(
        ...
        let duration_histogram =
            DurationHistogram::new(buckets).map_err(EventMetricsConfigError::from)?;
        Ok(Self {
            state: Arc::new(Mutex::new(EventMetricsState::new(duration_histogram))),
        })
    }
    ...
    pub fn snapshot(&self) -> EventMetricsSnapshot {
        ...
        EventMetricsSnapshot::from_state(
            state.in_flight,
            state.started,
            state.completed,
    ...
    pub(super) fn lock_state(&self) -> MutexGuard<'_, EventMetricsState> {
        self.state
            .lock()
...
impl From<DurationHistogramConfigError> for EventMetricsConfigError {
