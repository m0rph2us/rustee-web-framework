//! `PostgreSQL` persistence for AI batch artifact reservations and reconciliation state.
...
use crate::model::{PendingAiBatchArtifact, PendingAiBatchArtifactLimit};
...
/// Adds the durable identifier constraints introduced after the initial ledger migration.
///
/// Apply this after [`AI_BATCH_ARTIFACT_LEDGER_MIGRATION_SQL`]. It rejects the migration when an
/// existing row does not meet the bounded ASCII identifier contract, so operators must explicitly
/// repair or remove that row before declaring the deployment ready.
pub const AI_BATCH_ARTIFACT_LEDGER_IDENTIFIER_BOUND_MIGRATION_SQL: &str =
    include_str!("../migrations/0002_rustee_ai_batch_artifact_ledger_identifier_bounds.sql");
...
fn pending_from_row(
...
) -> Result<PendingAiBatchArtifact, PostgresAiBatchArtifactLedgerError> {
    let scope = row
    ...
    .map_err(|_| PostgresAiBatchArtifactLedgerError::InvalidMetadata)?;
    Ok(PendingAiBatchArtifact::new(reference))
}
...
fn validate_reference(
    ...
    for value in [
        ...
        if value.is_empty() || value.contains('\0') || value.len() > MAX_BATCH_IDENTIFIER_BYTES {
