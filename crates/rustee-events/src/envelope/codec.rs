//! Bounded event-envelope JSON encoding and pre-decode byte admission.

use rustee_json::{BoundedJsonError, to_vec_bounded};
use serde::Serialize;

use super::{EnvelopeError, MAX_EVENT_ENVELOPE_BYTES};

pub(super) fn encode<T>(value: &T) -> Result<Vec<u8>, EnvelopeError>
where
    T: Serialize + ?Sized,
{
    match to_vec_bounded(value, MAX_EVENT_ENVELOPE_BYTES) {
...
pub(super) fn validate_envelope_bytes(bytes: &[u8]) -> Result<(), EnvelopeError> {
    if bytes.len() > MAX_EVENT_ENVELOPE_BYTES {
